...
class MyEnv(gym.Env):
    ...
    def step(self,action):
        ...
        if tuple(x_) in self.total_set:
            ...
            is_discriminate = check_for_error_condition(self.current_sample,protected_params[0], array_length)
            if is_discriminate:
                reward = reward_biasd
                self.biasd += 1
        ...
        if reward == 0:
            ...
            #mahalanobis_dist = np.sqrt(self.mcd.mahalanobis(np.array(self.current_sample).reshape(1, -1)))
            #mahalanobis_dist = mahalanobis_dist[0]
    ...
    def reset(self, options):
        ...
        #self.current_sample = X[24148].tolist()
