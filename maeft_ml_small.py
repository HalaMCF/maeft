...
class k_cluster:
    ...
    def check_for_error_condition(self, clf, t, sens, length):
        t = np.array([t])
        ...
        result = clf.predict(np.vstack(to_check, dtype=int))
    ...
    def cluster(self):
        ...
        clf = model.load_model("./model/{}/{}_catboost".format(self.dataset, self.dataset))
        
        dis_x = []
        dis_kmeans = []
        for i in range(len(X)):
            temp = X[i].tolist()
            if self.check_for_error_condition(clf, temp, self.protected_params[0], self.array_length):
                dis_x.append(i)
